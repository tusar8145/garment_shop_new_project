...
const MatxVerticalNav = ({ items }) => {
  ...
  const renderLevels = (data) => {
    ...
    return data.map((item, index) => {
      var error = 0
      console.log(item.name, 'item.name', user)
      ...
      if (item.name == 'POD Upload' && user.is_scan_pod == 1) { error = 1 }
      if (item.name == 'New Pickup' && user.is_pickup == 0) { error = 1 }
      if (item.name == 'Unpaid Request' && user.is_authorize == 0) { error = 1 } 
      ...
      if (item.name == 'Deliverable' && user.is_update_status == 0 && user.is_pickup == 0) { error = 1 }


      if (item.name == 'All Report' && user.is_all_report == 0) { error = 1 }
      ...
      if (error == 0) {
        ...
        if (item.path == process.env.REACT_APP_BACKOFFICE_URL) {
          item.path = process.env.REACT_APP_BACKOFFICE_URL + '?' + window.localStorage.getItem('authorization').slice(-10);
        }
